...
func (o *Integration) GetDeviceIntegrations() DeviceIntegrationSlice {
	...
	return o.R.GetDeviceIntegrations()
}
...
func (integrationL) LoadDeviceIntegrations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeIntegration interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`device_definitions_api.device_integrations.integration_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Integration) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(integrationAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(integrationPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert integrations, could not build conflict column list")
			}

			conflict = make([]string, len(integrationPrimaryKeyColumns))
			copy(conflict, integrationPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"device_definitions_api\".\"integrations\"", updateOnConflict, ret, update, conflict, insert, opts...)
