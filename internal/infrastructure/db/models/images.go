...
func Images(mods ...qm.QueryMod) imageQuery {
	mods = append(mods, qm.From("\"device_definitions_api\".\"images\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"device_definitions_api\".\"images\".*"})
	}
...
func FindImage(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Image, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"device_definitions_api\".\"images\" where \"id\"=$1", sel,
	)
...
func (o *Image) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"device_definitions_api\".\"images\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"device_definitions_api\".\"images\" %sDEFAULT VALUES%s"
		}
...
func (o *Image) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"device_definitions_api\".\"images\" SET %s WHERE %s",
...
func (o ImageSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"device_definitions_api\".\"images\" SET %s WHERE %s",
...
func (o *Image) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"device_definitions_api\".\"images\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *Image) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"device_definitions_api\".\"images\" WHERE \"id\"=$1"
...
func (o ImageSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"device_definitions_api\".\"images\" WHERE " +
...
func (o *ImageSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"device_definitions_api\".\"images\".* FROM \"device_definitions_api\".\"images\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, imagePrimaryKeyColumns, len(*o))
...
func ImageExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"device_definitions_api\".\"images\" where \"id\"=$1 limit 1)"
