...
func (deviceIntegrationL) LoadIntegration(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDeviceIntegration interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		args[object.IntegrationID] = struct{}{}
	...
	query := NewQuery(
		...
		qm.WhereIn(`device_definitions_api.integrations.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *DeviceIntegration) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(deviceIntegrationAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(deviceIntegrationPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert device_integrations, could not build conflict column list")
			}

			conflict = make([]string, len(deviceIntegrationPrimaryKeyColumns))
			copy(conflict, deviceIntegrationPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"device_definitions_api\".\"device_integrations\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *DeviceIntegration) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return DeviceIntegrationExists(ctx, exec, o.DeviceDefinitionID, o.IntegrationID, o.Region)
}
