...
func (o *DeviceNhtsaRecall) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(deviceNhtsaRecallAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(deviceNhtsaRecallPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert device_nhtsa_recalls, could not build conflict column list")
			}

			conflict = make([]string, len(deviceNhtsaRecallPrimaryKeyColumns))
			copy(conflict, deviceNhtsaRecallPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"device_definitions_api\".\"device_nhtsa_recalls\"", updateOnConflict, ret, update, conflict, insert, opts...)
