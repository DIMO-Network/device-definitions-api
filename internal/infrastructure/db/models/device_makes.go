...
var DeviceMakeWhere = struct {
	...
	HardwareTemplateID: whereHelpernull_String{field: "\"device_definitions_api\".\"device_makes\".\"hardware_template_id\""},
}
...
func (deviceMakeL) LoadDeviceDefinitions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDeviceMake interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`device_definitions_api.device_definitions.device_make_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (deviceMakeL) LoadVinNumbers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDeviceMake interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`device_definitions_api.vin_numbers.device_make_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (deviceMakeL) LoadWmis(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDeviceMake interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`device_definitions_api.wmis.device_make_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *DeviceMake) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(deviceMakeAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(deviceMakePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert device_makes, could not build conflict column list")
			}

			conflict = make([]string, len(deviceMakePrimaryKeyColumns))
			copy(conflict, deviceMakePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"device_definitions_api\".\"device_makes\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *DeviceMake) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return DeviceMakeExists(ctx, exec, o.ID)
}
