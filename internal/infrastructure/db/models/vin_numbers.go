...
var VinNumberWhere = struct {
	...
	PowertrainType:   whereHelpernull_String{field: "\"device_definitions_api\".\"vin_numbers\".\"powertrain_type\""},
}
...
var (
	vinNumberAllColumns            = []string{"vin", "wmi", "vds", "check_digit", "serial_number", "vis", "created_at", "updated_at", "style_id", "decode_provider", "year", "vincario_data", "drivly_data", "autoiso_data", "datgroup_data", "definition_id", "manufacturer_name", "vin17_data", "powertrain_type"}
	vinNumberColumnsWithoutDefault = []string{"vin", "serial_number", "definition_id", "manufacturer_name"}
	vinNumberColumnsWithDefault    = []string{"wmi", "vds", "check_digit", "vis", "created_at", "updated_at", "style_id", "decode_provider", "year", "vincario_data", "drivly_data", "autoiso_data", "datgroup_data", "vin17_data", "powertrain_type"}
...
func (vinNumberL) LoadStyle(ctx context.Context, e boil.ContextExecutor, singular bool, maybeVinNumber interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`device_definitions_api.device_styles.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *VinNumber) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(vinNumberAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(vinNumberPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert vin_numbers, could not build conflict column list")
			}

			conflict = make([]string, len(vinNumberPrimaryKeyColumns))
			copy(conflict, vinNumberPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"device_definitions_api\".\"vin_numbers\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *VinNumber) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return VinNumberExists(ctx, exec, o.Vin)
}
