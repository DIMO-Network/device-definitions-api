...
func (o *IntegrationFeature) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(integrationFeatureAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(integrationFeaturePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert integration_features, could not build conflict column list")
			}

			conflict = make([]string, len(integrationFeaturePrimaryKeyColumns))
			copy(conflict, integrationFeaturePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"device_definitions_api\".\"integration_features\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *IntegrationFeature) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return IntegrationFeatureExists(ctx, exec, o.FeatureKey)
}
