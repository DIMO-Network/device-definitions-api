...
func Run(ctx context.Context, logger zerolog.Logger, settings *config.Settings) {
	...
	m, _ := mediator.New(
		...
		mediator.WithHandler(&queries.GetAllDevicesMakeModelYearQuery{}, queries.NewGetAllDevicesMakeModelYearQueryHandler(deviceDefinitionRepository)),
		...
		mediator.WithHandler(&commands.CreateDeviceIntegrationCommand{}, commands.NewCreateDeviceIntegrationCommandHandler(deviceIntegrationRepository, pdb.DBS)),
