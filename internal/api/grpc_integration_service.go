...
func (s *GrpcIntegrationService) GetCompatibilitiesByMake(ctx context.Context, in *p_grpc.GetCompatibilitiesByMakeRequest) (*p_grpc.GetCompatibilitiesByMakeResponse, error) {
	//logger := s.logger.With().Str("rpc", "GetCompatibilitiesByMake").Logger()
	...
	// move stuff in here to query layer, just return what we need.
	return result, nil

	//result := &p_grpc.GetCompatibilitiesByMakeResponse{}
	//
